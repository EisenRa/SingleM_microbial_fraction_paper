...
comparison <- rbind(rf1, rf2) %>%
  mutate(database = str_replace_all(database, "_mag_sizes", "_supplemented"))

comparison %>%
  ggplot(aes(x = sample, y = read_fraction_full * 100, colour = database, group = sample)) +
  geom_point(size = 4) +
...
ggsave("figures/SI_figure2.png", width = 10, height = 7.5)



################################################################################
## Extra: Replotting Figure 3 with the updated SMF estimates
################################################################################
...
combined_hyenas + plot_annotation(tag_levels = "A") &
